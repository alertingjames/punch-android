package com.unitedwebspace.punchcard.Entities;

import java.util.HashMap;
import java.util.Map;

/**
 * Created by sonback123456 on 4/24/2018.
 */

public class Font {
    String[] fontTexts = {
            "AcademyEngravedLetPlain",
            "AdlantaRegular",
            "AdlantaLight",
            "AlexBrush-Regular",
            "AlNile",
            "AlNile-Bold",
            "AmericanTypewriter-CondensedBold",
            "AmericanTypewriter-Condensed",
            "AmericanTypewriter-CondensedLight",
            "AmericanTypewriter",
            "AmericanTypewriter-Bold",
            "AmericanTypewriter-Semibold",
            "AmericanTypewriter-Light",
            "AppleColorEmoji",
            "Attic",
            "AppleSDGothicNeo-Thin",
            "AppleSDGothicNeo-Light",
            "AppleSDGothicNeo-Regular",
            "AppleSDGothicNeo-Bold",
            "AppleSDGothicNeo-SemiBold",
            "AppleSDGothicNeo-UltraLight",
            "AppleSDGothicNeo-Medium",
            "Arellion", "Arial-BoldMT",
            "Arial-BoldItalicMT",
            "Arial-ItalicMT",
            "ArialRoundedMTBold",
            "ArialHebrew-Bold",
            "ArialHebrew-Light",
            "ArialHebrew",
            "ArialMT",
            "Arizonia-Regular",
            "ArvinRegular-@",
            "Arvin-Bold",
            "Arvin-BoldItalic",
            "Arvin-Italic",
            "ArvinLight-@",
            "ArvinLight-Italic",
            "Avenir-Oblique",
            "Avenir-HeavyOblique",
            "Avenir-Heavy",
            "Avenir-BlackOblique",
            "Avenir-BookOblique",
            "Avenir-Roman",
            "Avenir-Medium",
            "Avenir-Black",
            "Avenir-Light",
            "Avenir-MediumOblique",
            "Avenir-Book",
            "Avenir-LightOblique",
            "AvenueX-Regular",
            "AvenirNext-Medium",
            "AvenirNext-DemiBoldItalic",
            "AvenirNext-DemiBold",
            "AvenirNext-HeavyItalic",
            "AvenirNext-Regular",
            "AvenirNext-Italic",
            "AvenirNext-MediumItalic",
            "AvenirNext-UltraLightItalic",
            "AvenirNext-BoldItalic",
            "AvenirNext-Heavy",
            "AvenirNext-Bold",
            "AvenirNext-UltraLight",
            "AvenirNextCondensed-Heavy",
            "AvenirNextCondensed-MediumItalic",
            "AvenirNextCondensed-Regular",
            "AvenirNextCondensed-UltraLightItalic",
            "AvenirNextCondensed-Medium",
            "AvenirNextCondensed-HeavyItalic",
            "AvenirNextCondensed-DemiBoldItalic",
            "AvenirNextCondensed-Bold",
            "AvenirNextCondensed-DemiBold",
            "AvenirNextCondensed-BoldItalic",
            "AvenirNextCondensed-Italic",
            "AvenirNextCondensed-UltraLight",
            "Baskerville-SemiBoldItalic",
            "Baskerville-SemiBold",
            "Baskerville-BoldItalic",
            "Baskerville",
            "Baskerville-Bold",
            "Baskerville-Italic",
            "BodoniSvtyTwoSCITCTT-Book",
            "BodoniSvtyTwoOSITCTT-BookIt",
            "BodoniSvtyTwoOSITCTT-Book",
            "BodoniSvtyTwoOSITCTT-Bold",
            "BodoniSvtyTwoITCTT-Bold",
            "BodoniSvtyTwoITCTT-BookIta",
            "BodoniSvtyTwoITCTT-Book",
            "BradleyHandITCTT-Bold",
            "Bungasai",
            "ButterflyKids",
            "ChalkboardSE-Bold",
            "ChalkboardSE-Light",
            "ChalkboardSE-Regular",
            "Chalkduster",
            "Charter-BlackItalic",
            "Charter-Bold",
            "Charter-Roman",
            "Charter-Black",
            "Charter-BoldItalic",
            "Charter-Italic",
            "Cochin-Italic",
            "Cochin-Bold",
            "Cochin",
            "Cochin-BoldItalic",
            "Copperplate-Light",
            "Copperplate",
            "Copperplate-Bold",
            "Courier-BoldOblique",
            "Courier-Oblique",
            "Courier",
            "Courier-Bold",
            "CourierNewPS-ItalicMT",
            "CourierNewPSMT",
            "CourierNewPS-BoldItalicMT",
            "CourierNewPS-BoldMT",
            "DamascusBold",
            "DamascusLight",
            "Damascus",
            "DamascusMedium",
            "DamascusSemiBold",
            "DevanagariSangamMN",
            "DevanagariSangamMN-Bold",
            "DidactGothic-Regular",
            "Didot-Bold",
            "Didot",
            "Didot-Italic",
            "DINAlternate-Bold",
            "DINCondensed-Bold",
            "DiwanMishafi",
            "Elley",
            "Eltercerhombre-Normal",
            "EuphemiaUCAS",
            "EuphemiaUCAS-Italic",
            "EuphemiaUCAS-Bold",
            "Ewert-Regular",
            "FashionVictim",
            "Farah",
            "Ferrum",
            "Futura-CondensedExtraBold",
            "Futura-Medium",
            "Futura-Bold",
            "Futura-CondensedMedium",
            "Futura-MediumItalic",
            "GeezaPro-Bold",
            "GeezaPro",
            "Georgia-BoldItalic",
            "Georgia-Italic",
            "Georgia",
            "Georgia-Bold",
            "GFSDidot-Regular",
            "GillSans-Italic",
            "GillSans-SemiBold",
            "GillSans-UltraBold",
            "GillSans-Light",
            "GillSans-Bold",
            "GillSans",
            "GillSans-SemiBoldItalic",
            "GillSans-BoldItalic",
            "GillSans-LightItalic",
            "GreatVibes-Regular",
            "GujaratiSangamMN",
            "GujaratiSangamMN-Bold",
            "GurmukhiMN-Bold",
            "GurmukhiMN",
            "Helvetica-Oblique",
            "Helvetica-BoldOblique",
            "Helvetica",
            "Helvetica-Light",
            "Helvetica-Bold",
            "Helvetica-LightOblique",
            "HelveticaNeue-UltraLightItalic",
            "HelveticaNeue-Medium",
            "HelveticaNeue-MediumItalic",
            "HelveticaNeue-UltraLight",
            "HelveticaNeue-Italic",
            "HelveticaNeue-Light",
            "HelveticaNeue-ThinItalic",
            "HelveticaNeue-LightItalic",
            "HelveticaNeue-Bold",
            "HelveticaNeue-Thin",
            "HelveticaNeue-CondensedBlack",
            "HelveticaNeue",
            "HelveticaNeue-CondensedBold",
            "HelveticaNeue-BoldItalic",
            "HiraginoSans-W3",
            "HiraginoSans-W6",
            "HiraMaruProN-W4",
            "HiraMinProN-W3",
            "HiraMinProN-W6",
            "HoeflerText-Italic",
            "HoeflerText-Black",
            "HoeflerText-Regular",
            "HoeflerText-BlackItalic",
            "Italiana-Regular",
            "JW_Script",
            "Kailasa-Bold",
            "Kailasa",
            "KannadaSangamMN-Bold",
            "KannadaSangamMN",
            "Kefa-Regular",
            "KhmerSangamMN",
            "KohinoorDevanagari-Regular",
            "KohinoorDevanagari-Light",
            "KohinoorDevanagari-Semibold",
            "KohinoorBangla-Regular",
            "KohinoorBangla-Semibold",
            "KohinoorBangla-Light",
            "KohinoorTelugu-Regular",
            "KohinoorTelugu-Medium",
            "KohinoorTelugu-Light",
            "LaoSangamMN",
            "LemonTuesday",
            "LibreBodoni-Bold",
            "LibreBodoni-Regular",
            "LibreBodoni-BoldItalic",
            "LibreBodoni-Italic",
            "LMMonoLt10-Regular",
            "LMMonoLt10-Bold",
            "LMMonoLt10-Oblique",
            "LMMonoLt10-BoldOblique",
            "LMMonoPropLt10-BoldOblique",
            "LMMonoPropLt10-Oblique",
            "LMMonoPropLt10-Bold",
            "LMMonoPropLt10-Regular",
            "LMMonoSlant10-Regular",
            "LMMono12-Regular",
            "LMMono8-Regular",
            "LMMono9-Regular",
            "LMMono10-Regular",
            "LMMono10-Italic",
            "LMMonoCaps10-Oblique",
            "LMMonoCaps10-Regular",
            "LMMonoProp10-Oblique",
            "LMMonoProp10-Regular",
            "LMMonoLtCond10-Oblique",
            "LMMonoLtCond10-Regular",
            "Lobster-Regular",
            "LucienSchoenschriftvCAT",
            "Magnolia-Script",
            "MalayalamSangamMN-Bold",
            "MalayalamSangamMN",
            "MarkerFelt-Thin",
            "MarkerFelt-Wide",
            "Medio",
            "Menlo-BoldItalic",
            "Menlo-Bold",
            "Menlo-Italic",
            "Menlo-Regular",
            "Messages",
            "MiamaNueva",
            "MrDeHaviland-Regular",
            "MyanmarSangamMN",
            "MyanmarSangamMN-Bold",
            "MysteryQuest",
            "Nandaka-Western",
            "Nickainley",
            "Noteworthy-Bold",
            "Noteworthy-Light",
            "NotoNastaliqUrdu",
            "Occupied",
            "Occupied-Italic",
            "Olives",
            "OlivesBold",
            "OlivesLight",
            "Optima-ExtraBlack",
            "Optima-BoldItalic",
            "Optima-Italic",
            "Optima-Regular",
            "Optima-Bold",
            "OriyaSangamMN",
            "OriyaSangamMN-Bold",
            "Palatino-Italic",
            "Palatino-Roman",
            "Palatino-BoldItalic",
            "Palatino-Bold",
            "Papyrus-Condensed",
            "Papyrus",
            "Parisish",
            "Parisienne-Regular",
            "PartyLetPlain",
            "PatPaCoolRegular",
            "Penna",
            "PingFangHK-Medium",
            "PingFangHK-Thin",
            "PingFangHK-Regular",
            "PingFangHK-Ultralight",
            "PingFangHK-Semibold",
            "PingFangHK-Light",
            "PingFangTC-Regular",
            "PingFangTC-Thin",
            "PingFangTC-Medium",
            "PingFangTC-Semibold",
            "PingFangTC-Light",
            "PingFangTC-Ultralight",
            "PingFangSC-Medium",
            "PingFangSC-Semibold",
            "PingFangSC-Light",
            "PingFangSC-Ultralight",
            "PingFangSC-Regular",
            "PingFangSC-Thin",
            "PinyonScript",
            "RiseofKingdom",
            "Rockwell-Italic",
            "Rockwell-Regular",
            "Rockwell-Bold",
            "Rockwell-BoldItalic",
            "RougeScript-Regular",
            "SavoyeLetPlain",
            "SinhalaSangamMN-Bold",
            "SinhalaSangamMN",
            "SnellRoundhand",
            "SnellRoundhand-Bold",
            "SnellRoundhand-Black",
            "SophiaNubian",
            "SophiaNubian-BoldItalic",
            "SophiaNubian-Bold",
            "SophiaNubian-Italic",
            "Soria-Soria", "Spirax-Regular", "Steamy", "Symbol",
            "TamilSangamMN",
            "TamilSangamMN-Bold",
            "Thonburi",
            "Thonburi-Light",
            "Thonburi-Bold",
            "TimesNewRomanPS-ItalicMT",
            "TimesNewRomanPS-BoldItalicMT",
            "TimesNewRomanPS-BoldMT",
            "TimesNewRomanPSMT",
            "ToThePoint",
            "TrebuchetMS-Bold",
            "TrebuchetMS-Italic",
            "Trebuchet-BoldItalic",
            "TrebuchetMS",
            "Trumpit",
            "VanchromeClear-Regular",
            "VanchromeFront-Regular",
            "VanchromeBack-Regular",
            "VanchromeOutline-Regular",
            "VanchromeEdge-Regular",
            "Verdana-Italic",
            "Verdana",
            "Verdana-Bold",
            "Verdana-BoldItalic",
            "Westmeath",
            "Westmeath-Italic",
            "ZapfDingbatsITC",
            "Zapfino",
            "Zero"
    };

    String[] fontPaths = {
            "AcademyEngravedLetPlain.ttf",
            "AdlantaRegular.otf",
            "AdlantaLight.otf",
            "AlexBrush-Regular.ttf",
            "AlNile.ttf",
            "AlNile-Bold.ttf",
            "AmericanTypewriter-CondensedBold.ttf",
            "AmericanTypewriter-Condensed.ttf",
            "AmericanTypewriter-CondensedLight.ttf",
            "AmericanTypewriter.ttf",
            "AmericanTypewriter-Bold.ttf",
            "AmericanTypewriter-Semibold.ttf",
            "AmericanTypewriter-Light.ttf",
            "AppleColorEmoji.ttf",
            "Attic.ttf",
            "AppleSDGothicNeo-Thin.ttf",
            "AppleSDGothicNeo-Light.ttf",
            "AppleSDGothicNeo-Regular.ttf",
            "AppleSDGothicNeo-Bold.otf",
            "AppleSDGothicNeo-SemiBold.ttf",
            "AppleSDGothicNeo-UltraLight.ttf",
            "AppleSDGothicNeo-Medium.ttf",
            "Arellion.otf",
            "Arial-BoldMT.ttf",
            "Arial-BoldItalicMT.otf",
            "Arial-ItalicMT.otf",
            "ArialRoundedMTBold.TTF",
            "ArialHebrew-Bold.ttf",
            "ArialHebrew-Light.ttf",
            "ArialHebrew.ttf",
            "ArialMT.otf",
            "Arizonia-Regular.ttf",
            "ArvinRegular-@.ttf",
            "Arvin-Bold.ttf",
            "Arvin-BoldItalic.ttf",
            "Arvin-Italic.ttf",
            "ArvinLight-@.ttf",
            "ArvinLight-Italic.ttf",
            "Avenir-Oblique.otf",
            "Avenir-HeavyOblique.otf",
            "Avenir-Heavy.otf",
            "Avenir-BlackOblique.ttf",
            "Avenir-BookOblique.ttf",
            "Avenir-Roman.ttf",
            "Avenir-Medium.ttf",
            "Avenir-Black.ttf",
            "Avenir-Light.ttf",
            "Avenir-MediumOblique.ttf",
            "Avenir-Book.otf",
            "Avenir-LightOblique.otf",
            "AvenueX-Regular.otf",
            "AvenirNext-Medium.ttf",
            "AvenirNext-DemiBoldItalic.ttf",
            "AvenirNext-DemiBold.ttf",
            "AvenirNext-HeavyItalic.ttf",
            "AvenirNext-Regular.ttf",
            "AvenirNext-Italic.ttf",
            "AvenirNext-MediumItalic.ttf",
            "AvenirNext-UltraLightItalic.ttf",
            "AvenirNext-BoldItalic.ttf",
            "AvenirNext-Heavy.ttf",
            "AvenirNext-Bold.ttf",
            "AvenirNext-UltraLight.ttf",
            "AvenirNextCondensed-Heavy.ttf",
            "AvenirNextCondensed-MediumItalic.ttf",
            "AvenirNextCondensed-Regular.ttf",
            "AvenirNextCondensed-UltraLightItalic.ttf",
            "AvenirNextCondensed-Medium.ttf",
            "AvenirNextCondensed-HeavyItalic.ttf",
            "AvenirNextCondensed-DemiBoldItalic.ttf",
            "AvenirNextCondensed-Bold.ttf",
            "AvenirNextCondensed-DemiBold.ttf",
            "AvenirNextCondensed-BoldItalic.ttf",
            "AvenirNextCondensed-Italic.ttf",
            "AvenirNextCondensed-UltraLight.ttf",
            "Baskerville-SemiBoldItalic.ttf",
            "Baskerville-SemiBold.ttf",
            "Baskerville-BoldItalic.ttf",
            "Baskerville.ttf",
            "Baskerville-Bold.ttf",
            "Baskerville-Italic.ttf",
            "BodoniSvtyTwoSCITCTT-Book.ttf",
            "BodoniSvtyTwoOSITCTT-BookIt.ttf",
            "BodoniSvtyTwoOSITCTT-Book.ttf",
            "BodoniSvtyTwoOSITCTT-Bold.ttf",
            "BodoniSvtyTwoITCTT-Bold.ttf",
            "BodoniSvtyTwoITCTT-BookIta.ttf",
            "BodoniSvtyTwoITCTT-Book.ttf",
            "BradleyHandITCTT-Bold.ttf",
            "Bungasai.ttf",
            "ButterflyKids.ttf",
            "ChalkboardSE-Bold.ttf",
            "ChalkboardSE-Light.ttf",
            "ChalkboardSE-Regular.ttf",
            "Chalkduster.ttf",
            "Charter-BlackItalic.otf",
            "Charter-Bold.otf",
            "Charter-Roman.ttf",
            "Charter-Black.otf",
            "Charter-BoldItalic.otf",
            "Charter-Italic.otf",
            "Cochin-Italic.otf",
            "Cochin-Bold.otf",
            "Cochin.ttf",
            "Cochin-BoldItalic.otf",
            "Copperplate-Light.ttf",
            "Copperplate.ttf",
            "Copperplate-Bold.ttf",
            "Courier-BoldOblique.otf",
            "Courier-Oblique.otf",
            "Courier.ttf",
            "Courier-Bold.ttf",
            "CourierNewPS-ItalicMT.ttf",
            "CourierNewPSMT.ttf",
            "CourierNewPS-BoldItalicMT.ttf",
            "CourierNewPS-BoldMT.ttf",
            "DamascusBold.ttf",
            "DamascusLight.ttf",
            "Damascus.ttf",
            "DamascusMedium.ttf",
            "DamascusSemiBold.ttf",
            "DevanagariSangamMN.ttf",
            "DevanagariSangamMN-Bold.ttf",
            "DidactGothic-Regular.ttf",
            "Didot-Bold.ttf",
            "Didot.ttf",
            "Didot-Italic.ttf",
            "DINAlternate-Bold.otf",
            "DINCondensed-Bold.ttf",
            "DiwanMishafi.ttf",
            "Elley.otf",
            "Eltercerhombre-Normal.ttf",
            "EuphemiaUCAS.ttf",
            "EuphemiaUCAS-Italic.ttf",
            "EuphemiaUCAS-Bold.ttf",
            "Ewert-Regular.ttf",
            "FashionVictim.ttf",
            "Farah.ttf",
            "Ferrum.otf",
            "Futura-CondensedExtraBold.otf",
            "Futura-Medium.otf",
            "Futura-Bold.otf",
            "Futura-CondensedMedium.otf",
            "Futura-MediumItalic.ttf",
            "GeezaPro-Bold.ttf",
            "GeezaPro.ttf",
            "Georgia-BoldItalic.ttf",
            "Georgia-Italic.ttf",
            "Georgia.ttf",
            "Georgia-Bold.ttf",
            "GFSDidot-Regular.ttf",
            "GillSans-Italic.otf",
            "GillSans-SemiBold.otf",
            "GillSans-UltraBold.ttf",
            "GillSans-Light.otf",
            "GillSans-Bold.otf",
            "GillSans.otf",
            "GillSans-SemiBoldItalic.otf",
            "GillSans-BoldItalic.otf",
            "GillSans-LightItalic.otf",
            "GreatVibes-Regular.ttf",
            "GujaratiSangamMN.ttf",
            "GujaratiSangamMN-Bold.ttf",
            "GurmukhiMN-Bold.ttf",
            "GurmukhiMN.ttf",
            "Helvetica-Oblique.ttf",
            "Helvetica-BoldOblique.ttf",
            "Helvetica.ttf",
            "Helvetica-Light.ttf",
            "Helvetica-Bold.ttf",
            "Helvetica-LightOblique.ttf",
            "HelveticaNeue-UltraLightItalic.otf",
            "HelveticaNeue-Medium.ttf",
            "HelveticaNeue-MediumItalic.ttf",
            "HelveticaNeue-UltraLight.otf",
            "HelveticaNeue-Italic.ttf",
            "HelveticaNeue-Light.ttf",
            "HelveticaNeue-ThinItalic.otf",
            "HelveticaNeue-LightItalic.ttf",
            "HelveticaNeue-Bold.ttf",
            "HelveticaNeue-Thin.ttf",
            "HelveticaNeue-CondensedBlack.otf",
            "HelveticaNeue.ttf",
            "HelveticaNeue-CondensedBold.otf",
            "HelveticaNeue-BoldItalic.ttf",
            "HiraginoSans-W3.otf",
            "HiraginoSans-W6.otf",
            "HiraMaruProN-W4.otf",
            "HiraMinProN-W3.otf",
            "HiraMinProN-W6.otf",
            "HoeflerText-Italic.ttf",
            "HoeflerText-Black.ttf",
            "HoeflerText-Regular.ttf",
            "HoeflerText-BlackItalic.ttf",
            "Italiana-Regular.ttf",
            "JW_Script.ttf",
            "Kailasa-Bold.ttf",
            "Kailasa.ttf",
            "KannadaSangamMN-Bold.ttf",
            "KannadaSangamMN.ttf",
            "Kefa-Regular.ttf",
            "KhmerSangamMN.ttf",
            "KohinoorDevanagari-Regular.ttf",
            "KohinoorDevanagari-Light.ttf",
            "KohinoorDevanagari-Semibold.ttf",
            "KohinoorBangla-Regular.ttf",
            "KohinoorBangla-Semibold.ttf",
            "KohinoorBangla-Light.ttf",
            "KohinoorTelugu-Regular.ttf",
            "KohinoorTelugu-Medium.ttf",
            "KohinoorTelugu-Light.ttf",
            "LaoSangamMN.ttf",
            "LemonTuesday.otf",
            "LibreBodoni-Bold.ttf",
            "LibreBodoni-Regular.ttf",
            "LibreBodoni-BoldItalic.ttf",
            "LibreBodoni-Italic.ttf",
            "LMMonoLt10-Regular.otf",
            "LMMonoLt10-Bold.otf",
            "LMMonoLt10-Oblique.otf",
            "LMMonoLt10-BoldOblique.otf",
            "LMMonoPropLt10-BoldOblique.otf",
            "LMMonoPropLt10-Oblique.otf",
            "LMMonoPropLt10-Bold.otf",
            "LMMonoPropLt10-Regular.otf",
            "LMMonoSlant10-Regular.otf",
            "LMMono12-Regular.otf",
            "LMMono8-Regular.otf",
            "LMMono9-Regular.otf",
            "LMMono10-Regular.otf",
            "LMMono10-Italic.otf",
            "LMMonoCaps10-Oblique.otf",
            "LMMonoCaps10-Regular.otf",
            "LMMonoProp10-Oblique.otf",
            "LMMonoProp10-Regular.otf",
            "LMMonoLtCond10-Oblique.otf",
            "LMMonoLtCond10-Regular.otf",
            "Lobster-Regular.ttf",
            "LucienSchoenschriftvCAT.ttf",
            "Magnolia-Script.otf",
            "MalayalamSangamMN-Bold.ttf",
            "MalayalamSangamMN.ttf",
            "MarkerFelt-Thin.ttf",
            "MarkerFelt-Wide.ttf",
            "Medio.otf",
            "Menlo-BoldItalic.ttf",
            "Menlo-Bold.ttf",
            "Menlo-Italic.ttf",
            "Menlo-Regular.ttf",
            "Messages.ttf",
            "MiamaNueva.otf",
            "MrDeHaviland-Regular.ttf",
            "MyanmarSangamMN.ttf",
            "MyanmarSangamMN-Bold.ttf",
            "MysteryQuest.ttf",
            "Nandaka-Western.ttf",
            "Nickainley.ttf",
            "Noteworthy-Bold.ttf",
            "Noteworthy-Light.ttf",
            "NotoNastaliqUrdu.ttf",
            "Occupied.otf",
            "Occupied-Italic.otf",
            "Olives.ttf",
            "OlivesBold.ttf",
            "OlivesLight.ttf",
            "Optima-ExtraBlack.ttf",
            "Optima-BoldItalic.ttf",
            "Optima-Italic.ttf",
            "Optima-Regular.ttf",
            "Optima-Bold.ttf",
            "OriyaSangamMN.ttf",
            "OriyaSangamMN-Bold.ttf",
            "Palatino-Italic.ttf",
            "Palatino-Roman.ttf",
            "Palatino-BoldItalic.TTF",
            "Palatino-Bold.ttf",
            "Papyrus-Condensed.ttf",
            "Papyrus.TTF",
            "Parisish.ttf",
            "Parisienne-Regular.ttf",
            "PartyLetPlain.ttf",
            "PatPaCoolRegular.otf",
            "Penna.otf",
            "PingFangHK-Medium.ttf",
            "PingFangHK-Thin.ttf",
            "PingFangHK-Regular.otf",
            "PingFangHK-Ultralight.ttf",
            "PingFangHK-Semibold.ttf",
            "PingFangHK-Light.TTF",
            "PingFangTC-Regular.ttf",
            "PingFangTC-Thin.ttf",
            "PingFangTC-Medium.ttf",
            "PingFangTC-Semibold.ttf",
            "PingFangTC-Light.ttf",
            "PingFangTC-Ultralight.ttf",
            "PingFangSC-Medium.ttf",
            "PingFangSC-Semibold.otf",
            "PingFangSC-Light.TTF",
            "PingFangSC-Ultralight.ttf",
            "PingFangSC-Regular.ttf",
            "PingFangSC-Thin.ttf",
            "PinyonScript.ttf",
            "RiseofKingdom.ttf",
            "Rockwell-Italic.otf",
            "Rockwell-Regular.ttf",
            "Rockwell-Bold.ttf",
            "Rockwell-BoldItalic.otf",
            "RougeScript-Regular.ttf",
            "SavoyeLetPlain.otf",
            "SinhalaSangamMN-Bold.ttf",
            "SinhalaSangamMN.ttf",
            "SnellRoundhand.ttf",
            "SnellRoundhand-Bold.ttf",
            "SnellRoundhand-Black.ttf",
            "SophiaNubian.ttf",
            "SophiaNubian-BoldItalic.ttf",
            "SophiaNubian-Bold.ttf",
            "SophiaNubian-Italic.ttf",
            "Soria-Soria.ttf",
            "Spirax-Regular.ttf",
            "Steamy.otf",
            "Symbol.ttf",
            "TamilSangamMN.TTF",
            "TamilSangamMN-Bold.ttf",
            "Thonburi.otf",
            "Thonburi-Light.otf",
            "Thonburi-Bold.ttf",
            "TimesNewRomanPS-ItalicMT.ttf",
            "TimesNewRomanPS-BoldItalicMT.ttf",
            "TimesNewRomanPS-BoldMT.ttf",
            "TimesNewRomanPSMT.ttf",
            "ToThePoint.ttf",
            "TrebuchetMS-Bold.ttf",
            "TrebuchetMS-Italic.ttf",
            "Trebuchet-BoldItalic.ttf",
            "TrebuchetMS.ttf",
            "Trumpit.otf",
            "VanchromeClear-Regular.ttf",
            "VanchromeFront-Regular.ttf",
            "VanchromeBack-Regular.ttf",
            "VanchromeOutline-Regular.ttf",
            "VanchromeEdge-Regular.ttf",
            "Verdana-Italic.ttf",
            "Verdana.ttf",
            "Verdana-Bold.ttf",
            "Verdana-BoldItalic.ttf",
            "Westmeath.ttf",
            "Westmeath-Italic.ttf",
            "ZapfDingbatsITC.ttf",
            "Zapfino.ttf",
            "Zero.ttf"
    };

    Map<String,String> fontMap = new HashMap<>();

    public Font(){

    }

    public String getFontPathFromMap(String fontName){
        String path="";
        for(int i=0; i<getFontTexts().length; i++){
            getFontMap().put(getFontTexts()[i], getFontPaths()[i]);
        }
        path = getFontMap().get(fontName);
        return path;
    }

    public void setFontMap(Map<String, String> fontMap) {
        this.fontMap = fontMap;
    }

    public Map<String, String> getFontMap() {
        return fontMap;
    }

    public void setFontTexts(String[] fontTexts) {
        this.fontTexts = fontTexts;
    }

    public void setFontPaths(String[] fontPaths) {
        this.fontPaths = fontPaths;
    }

    public String[] getFontTexts() {
        return fontTexts;
    }

    public String[] getFontPaths() {
        return fontPaths;
    }
}
