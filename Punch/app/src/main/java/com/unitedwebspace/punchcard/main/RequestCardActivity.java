...
public class RequestCardActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Card card = Commons.card;
        backgroundImage.setImageUrl(card.getBackgroundImage(), PunchApplication.getInstance().getImageLoader());

        phoneNumber.setText(card.getPhoneNumber());
        websiteUrl.setText(card.getWebsiteUrl());
        socialMediaLink.setText(card.getSocialMediaLink());
        notes.setText(card.getNotes());

        bsName.setText(card.getBusinessName());
        rwName.setText(card.getRewardName());
        businessName.setText(card.getBusinessName());
        rewardName.setText(card.getRewardName());
        if(card.getLogo().length() > 0)
            cardLogoNet.setImageUrl(card.getLogo(), PunchApplication.getInstance().getImageLoader());

        try{
            Typeface font = Typeface.createFromAsset(getAssets(), "font/"+(new Font().getFontPathFromMap(card.getBusinessFont())));
            bsName.setTypeface(font);
            font = Typeface.createFromAsset(getAssets(), "font/"+(new Font().getFontPathFromMap(card.getRewardFont())));
            rwName.setTypeface(font);
        }catch (RuntimeException e){
            e.printStackTrace();
        }

        rewards.setText(String.valueOf(card.getRewards()));
    ...
    public void phoneCall(View view){
        startActivity(new Intent(Intent.ACTION_DIAL,
                Uri.parse("tel:" + Commons.card.getPhoneNumber().replace("+","").replace("-", ""))));

    }

    public void shareEmail(View view){
        Intent emailIntent = new Intent(Intent.ACTION_SENDTO, Uri.fromParts(
                "mailto",Commons.card.getEmail(), null));
    ...
    public void requestCard() {
        ...
        StringRequest post = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("business_id", String.valueOf(Commons.card.get_bidx()));
